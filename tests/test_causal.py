...
def test_seller_quality_score_range():
    ...
    assert df["seller_quality_score"].std() > 0.001, \
...
def test_ci_ordering():
    ...
    assert (df["cate_lo"] <= df["cate"] + 1e-9).all(), "cate_lo > cate for some sellers"
    assert (df["cate"]    <= df["cate_hi"] + 1e-9).all(), "cate > cate_hi for some sellers"
...
def test_narrative_not_empty():
    ...
    pct_ok = df["narrative"].str.len().gt(30).mean()
    assert pct_ok >= 0.8, f"Only {pct_ok:.1%} of narratives have >30 chars"

def test_narrative_no_error_strings():
    df = pd.read_parquet(FINAL_PATH)
    error_mask = df["narrative"].str.startswith("[Error", na=False)
    pct_errors = error_mask.mean()
    assert pct_errors < 0.2, \
