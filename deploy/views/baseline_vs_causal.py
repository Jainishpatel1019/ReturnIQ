...
def render(df: pd.DataFrame) -> None:
    ...
    # Sample for performance; keep random_state for reproducibility
    ...
    # Diagonal "no bias" reference line
    ...
    # numpy 2.0 compat: trapezoid replaces trapz
    _trapz = getattr(np, "trapezoid", np.trapz)
    auuc_val = _trapz(y_c / 100, x_c / 100) - 0.5
    st.markdown(
        f'<div class="glass-card" style="text-align: center;">'
        f'<div style="color: #58a6ff; font-size: 20px; font-weight: 700;">{auuc_val:.3f} AUUC Lift</div>'
        f'<div style="color: #8b949e; font-size: 13px;">Positive lift indicates the model effectively ranks sellers by their true causal impact on losses.</div>'
        f'</div>',
